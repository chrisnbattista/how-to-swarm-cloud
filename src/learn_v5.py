...
# TODO TOP LINE
# Design all figures
# Make deck of figures, put conclusion from each figure on its slides
# Evaluate weight of conclusions
# Abstract is lumped conclusions

# TODO: see if we are close to bifurcation point. ICs and or parameters may be super sensitive
# Ideally we don't find when system fails haha

# Can test transference to different ICs using synthetic data. Different than real world data test.
# That real world test looks at unmodeled forces
# Optimization process assumes unmodeled pieces

# TODO: reference this https://arxiv.org/pdf/1910.07291.pdf
...
if dataset_choice == None:
    ## Split data
    filepaths = serialize.list_worlds(
        f"../data/{spatial_dims}D/scaled-gravity")
    print(f'Number of training files: {len(filepaths)}')
else:
    filepaths = serialize.list_worlds(
        f"../data/{spatial_dims}D/{dataset_choice}")
...
def percentage_error(actual, predicted):
    ...
    if torch.numel(actual) == 1:
        return torch.nan_to_num(torch.abs((actual - predicted) / actual),
                                nan=0.0,
                                posinf=0.0,
                                neginf=0.0)
    else:
...
class GravityNet(nn.Module):
    def __init__(self,
        ...
        self.delta_G = 0.025  # TODO: decay this value over time
        self.delta_r = 0.05
        self.first_X = None
    ...
    def run_sim(self, X, steps, G, r, return_world=False):
        ...
        temp_world = worlds.World(
            ...
            forces=[
                lambda world, context: forces.newtons_law_of_gravitation(
                    world=world, G=G, r=r.detach().clone(), context=context)
            ],
            indicators=[
                lambda w: indicators.hamiltonian(
                    w,
                    global_potentials=[
                        lambda w: potentials.gravitational_potential_energy(
                            world=w, G=G)
                    ]),
    ...
    def compute_loss(self, actual, predicted, return_components=False):
        ...
        baseline = torch.tile(self.first_X[0, :], (actual.shape[0], 1))
        ##print(f'baseline: {baseline}\nactual: {actual}\npredicted: {predicted}')
        baselined_actual = torch.div(actual, baseline)
        baselined_predicted = torch.div(predicted, baseline)
        p_err = self.hyperparams['p_err_weight'] * percentage_error(
            baselined_actual[:, pos], baselined_predicted[:, pos])
        v_err = self.hyperparams['v_err_weight'] * percentage_error(
            baselined_actual[:, vel], baselined_predicted[:, vel])
        # h_err = self.hyperparams['h_err_weight'] * percentage_error(
        #     baselined_actual[:, ham], baselined_predicted[:, ham])
    ...
    def compute_grad(self, actual, delta=None):
        ...
        plus_G_history = self.run_sim(X=self.last_X,
                                      steps=self.last_steps,
                                      G=self.G + d_G,
                                      r=self.r)
        plus_G_loss = self.compute_loss(actual, plus_G_history)
        minus_G_history = self.run_sim(X=self.last_X,
                                       steps=self.last_steps,
                                       G=self.G - d_G,
                                       r=self.r)
        ...
        plus_r_history = self.run_sim(X=self.last_X,
                                      steps=self.last_steps,
                                      G=self.G,
                                      r=self.r + d_r)
        plus_r_loss = self.compute_loss(actual, plus_r_history)
        minus_r_history = self.run_sim(X=self.last_X,
                                       steps=self.last_steps,
                                       G=self.G,
                                       r=self.r - d_r)
        ...
        self.G.grad = Variable(((plus_G_loss - minus_G_loss).clone().detach() /
                                (d_G * 2)).type(torch.FloatTensor))
        self.r.grad = Variable(((plus_r_loss - minus_r_loss).clone().detach() /
                                (d_r * 2)).type(torch.FloatTensor))
...
hyperparams = {
    ...
    'p': 0.1,
    'segment_length':
    100,  # try lengthening segment. figure out why it changes from quadratic to linear learning rate. show zoom in during paper.
    'accuracy_threshold': 0.008,
    'p_err_weight': 1.0,
    'v_err_weight': 3.0,
    ##'h_err_weight': 0.1,
    'betas': (0.95, 0.9995)
...
#     'p': 0.1,
#     'segment_length':
#     20,  # try lengthening segment. figure out why it changes from quadratic to linear learning rate. show zoom in during paper.
...
try:
    ...
    model = GravityNet(G_guess=G_seed_guess,
    ...
    for epoch in range(2):
        ...
        for segment_i in range(
                int(world.n_timesteps / hyperparams['segment_length'])):
            segment_history = history[
                int(hyperparams['segment_length'] * segment_i *
                    world.n_agents):int(hyperparams['segment_length'] *
                                        (segment_i + 1) * world.n_agents), :]
            ...
            loss, l_comps = model.compute_loss(y,
                                               y_pred,
                                               return_components=True)
            print(
                f'### Epoch {epoch} | Step {segment_i} | Overall Loss: {loss}')
            ...
            ## Halting condition
            if model.compute_loss(y,
                                  y_pred) < hyperparams['accuracy_threshold']:
                print(
                    f"Accuracy threshold reached in iteration {segment_i} of epoch {epoch} - halting."
                )
                halt = True
                break
    ...
    if True:
        print("Logging run...")
        pandas.DataFrame(losses_over_time).to_csv(
            '../data/results/losses' + str(datetime.datetime.now()) +
            "_segment_train.csv")
        print("Running simulation based on learned parameters...")
        reconstructed_world = model.run_sim(
            X=world.get_history()[0:int(world.n_agents), :],
            steps=int(world.n_timesteps),
            G=model.G,
            r=model.r,
            return_world=True)
        ...
        viz.trace_trajectories(world, fig, ax, 'Ground Truth World History')
        fig.text(0.1, 0.1, "Timesteps: " + str(world.n_timesteps), fontsize=9)
        viz.trace_trajectories(reconstructed_world, fig2, ax2,
                               'Reconstructed World History')
        fig2.text(0.1,
                  0.1,
                  "Timesteps: " + str(reconstructed_world.n_timesteps),
                  fontsize=9)
        plt.show(block=True)